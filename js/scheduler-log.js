...
function setupEventListeners() {
    ...
    const searchInputs = document.querySelectorAll('#date-from, #date-to, #data-type, #status-type');
...
function initializeSearch() {
    ...
    if (dataTypeSelect) {
        dataTypeSelect.value = 'current-direction'; // 실측유향유속
    }
    
    if (statusTypeSelect) {
        statusTypeSelect.value = 'success'; // 성공
    }
}

function initializeStatusColors() {
    // Apply default text color to all status cells
    const statusCells = document.querySelectorAll('.table-cell:nth-child(5)');
    statusCells.forEach(cell => {
        cell.style.color = '#18181a';
    });
}
...
function handleSearch() {
    ...
    const statusType = statusTypeSelect ? statusTypeSelect.value : '';
    
    console.log('Search:', { dateFrom, dateTo, dataType, statusType });
    ...
    setTimeout(() => {
        ...
        updateTableWithSearchResults(dateFrom, dateTo, dataType, statusType);
    }, 1000);
}
...
function updateTableWithSearchResults(dateFrom, dateTo, dataType, statusType) {
    // This would typically update the table with new data from the API
    console.log('Updating table with search results:', { dateFrom, dateTo, dataType, statusType });
    ...
    // Re-initialize status colors for new data
    initializeStatusColors();
